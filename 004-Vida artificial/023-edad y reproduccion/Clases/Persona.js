class Persona extends Entidad{
    ...
    vive(){
        if(this.s > 80){
            ...
            if(this.e > 10){
                if(this.edad > 20 && this.edad > 50){
                    this.buscaPareja(personas)
                    this.mueve()
                }else{
                    this.d += this.va
                    this.a += this.d  
    ...
    dibuja(contexto){
        ...
        if(this.sexo == 0){
            ...
            contexto.fillStyle = "rgb(0,0,255)"
        }*/
    ...
    buscaPareja(personas){
        let mejorcandidato = Math.floor(Math.random()*personas.length)
        //console.log(mejorcandidato)
        let mejordistancia = 1000000
        
        this.a = anguloEntreDosPuntos(
            this.x, 
            this.y, 
            personas[mejorcandidato].x, 
            personas[mejorcandidato].y
        ) 
        if(calculaDistancia(
            this.x,
            this.y,
            ...
            //console.log("como")
            this.reproduce(mejorcandidato)
        }
        
    }
    reproduce(mejorcandidato){
        //console.log("reproduccion!")
        this.reproducido = true
        personas[mejorcandidato].reproducido = true
        
        for(let i = 0;i<2;i++){
            personas.push(new Persona(this.va))
        }
